...
class SportgroupController extends Controller
{
    ...
    public function actionCreate($copy = NULL)
    {
        ...
        if (isset($_POST['Sportgroup'])) {
            $this->_model->attributes = $_POST['Sportgroup'];
            if ($this->_model->save()) {
    ...
    public function actionIndex()
    {
        ...
        $this->_pagesize = $userProfile->sportgroup_pagesize;
        $this->buildPageOptions();

        $this->_filter = new Sportgroup('search');
        $this->_filter->unsetAttributes(); // clear any default values
        if (isset($_GET['Sportgroup'])) $this->_filter->attributes = $_GET['Sportgroup'];

        $this->render('index', array(
            'dataProvider' => Sportgroup::model()->getAll($userProfile),
        ));
    }
     ...
     * @return Sportgroup the loaded model
    ...
    public function loadModel($id = NULL)
    {
        ...
        if ($this->_model === NULL) {
            $this->_model = Sportgroup::model()->findbyPk($id);
     ...
     * @param Sportgroup $model the model to be validated
     */
